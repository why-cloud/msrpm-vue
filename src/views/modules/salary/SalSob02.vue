    ...
    <div>
        ...
        <div style="margin-top: 10px">
            <el-table :data="salaries" border stripe>
                ...
                <el-table-column width="80" prop="createDate" label="启用时间"></el-table-column>
  ...
  methods: {
    ...
    deleteSalary (data) {
      this.$confirm('此操作将删除【' + data.name + '】账套，是否继续？', '提示', {
        ...
        this.deleteRequest('/es/salary' + data.id).then(resp => {
          if (resp) {
            this.initSalaries()
          }
        })
    ...
    nextStep () {
      if (this.activeItemIndex === 10) {
        if (this.salary.id) {
          this.putRequest('/es/salary', this.salary).then(resp => {
            if (resp) {
              this.initSalaries()
              this.dialogVisible = false
            }
          ...
          this.postRequest('/es/salary', this.salary).then(resp => {
            if (resp) {
              this.initSalaries()
              this.dialogVisible = false
            }
    ...
    initSalaries () {
      this.getRequest('/es/salary').then(resp => {
        if (resp) {
          this.salaries = resp
        }
      })
