<template>
  <div>
    <div id="myChart4" :style="{width: '1000px', height: '400px'}"></div>
     <div id="pieChart4" :style="{width: '1000px', height: '400px',border:'1px black solid'}"></div>
  </div>
</template>
<script>
  // 引入基本模板
...
export default {
    ...
    methods: {
      ...
      // 绘制图表
        ei.getPositionGenderCountOfEmp().then(response => {
          ...
          myChart.setOption({
            color: ['#91C7AE', '#38A7F0', '#DE7A66', '#e5323e'],
            title: {
              text: '员工职位统计',
              subtext: ''
            },
      ...
      piecharts () {
        let myChart = echarts.init(document.getElementById('pieChart4'))
      // 绘制图表
        ei.getPositionCountOfEmp().then(response => {
